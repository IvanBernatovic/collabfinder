      ...
      <div class="w-[365px]">
        <form @submit.prevent="submit">
          ...
          <form-group class="mb-6">
            ...
            <UInput size="xl" v-model="form.email" placeholder="Email" id="email" name="email" type="email" required
              aria-required="true" />
          ...
          <form-group>
            ...
            <UInput size="xl" v-model="form.password" placeholder="Password" :type="password_type" id="password"
              name="password" required aria-required="true" minlength="8" autocomplete="current-password" />
            <InputError :message="form.errors.password" />
          </form-group>

          <div class="flex justify-between mb-[40px] mt-4">
            <div class="flex items-center">
              <input type="checkbox" id="remember_me" name="remember_me" v-model="form.remember" class="w-4 h-4 mr-2" />

              <label class="text-gray-700 dark:text-white" for="remember_me">Remember me</label>
            </div>
            <a href="/forgot" class="font-medium">Forgot your password?</a>
          </div>

          <primary-button class="w-full mb-1" :disabled="form.processing">Sign in</primary-button>
        </form>

        <div class="text-center mt-2">
          New user?
          <Link href="/register" class="mx-auto">Sign up here</Link>
        </div>
